...
def find_overlapping_polygons(polygons, min_overlap_area):
    ...
    overlap_areas = []
    # polys_to_be_removed = []
    for i, poly1 in stqdm(enumerate(polygons)):
...
def sam_segmentation(sam, big_im, big_im_pred, coords, labels, min_area):
    ...
    h,w = big_im.shape[0], big_im.shape[1]
...
def plot_image_w_shape_color(image, all_grains, ax, grain_data, cmap='viridis'):
    ...
    #num_colors = len(shapes)  # Number of colors to choose
    #color_indices = index_of_images_of_particles_by_size_range(sizes_of_particles)
    # Get the individual colors
    #colors = [cmap(i) for i in color_indices]
    # fig = plt.figure(figsize=(10,10))
    # ax = fig.add_subplot(111)
    shapes = grain_data['shape'].unique()
    print("fzehfzuehfuzhfuz", shapes)
    color_map = {
        'clay':'red',
        'silt':'yellow',
        'gravel':'green',
    ...
    colors = [color_map[item] for item in shapes]
